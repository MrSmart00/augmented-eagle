...
const mockEmptySpeciesResponse = {
  flavor_text_entries: [],
  names: [],
};
...
describe("pokemonSpeciesApi", () => {
  ...
  describe("fetchPokemonFlavorText", () => {
    ...
    it("フレーバーテキストがない場合はnullを返す", async () => {
      globalThis.fetch = jest.fn().mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve(mockEmptySpeciesResponse),
      });
      const result = await fetchPokemonFlavorText(25);
    ...
    it("HTTPエラー時にエラーをスローする", async () => {
      ...
      await expect(fetchPokemonFlavorText(99999)).rejects.toThrow("Failed to fetch pokemon species: 404");
    });
  });
  ...
  describe("fetchPokemonSpeciesInfo", () => {
    ...
    it("HTTPエラー時にエラーをスローする", async () => {
      ...
      await expect(fetchPokemonSpeciesInfo(99999, "ja")).rejects.toThrow("Failed to fetch pokemon species: 404");
    });
  });
});
