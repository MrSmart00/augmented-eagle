...
const mockSearchPokemon: PokemonSummary[] = [
...
describe("useSearch", () => {
  it("初期状態では全てのポケモンが返される", () => {
    const { result } = renderHook(() => useSearch(mockSearchPokemon));

    expect(result.current.filteredItems).toEqual(mockSearchPokemon);
